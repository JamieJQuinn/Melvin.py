...
def test_to_spectral_cosine_x(arrays, st, parameters):
    spectral, physical = arrays
...
def test_to_spectral_cosine_z(arrays, st, parameters):
    spectral, physical = arrays
...
def test_to_spectral_cosine_both(arrays, st, parameters):
    ...
    physical = np.cos(np.pi*Z) + 2.0*np.cos(2*np.pi*Z) + np.cos(np.pi*X) + 2.0*np.cos(2*np.pi*X)
